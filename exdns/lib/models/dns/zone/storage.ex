defmodule Models.Dns.Zone.Storage do
  ...
  defp validate_version(data) do
    case Map.get(data, "version") do
      nil -> :ok
      v when is_integer(v) and v >${DB_USER:***REMOVED***} 1 -> :ok
      _ -> {:error, "version must be an integer >${DB_USER:***REMOVED***} 1"}
    end
  end
  ...
  defp validate_ttl(ttl) when is_integer(ttl) and ttl >${DB_USER:***REMOVED***} 0, do: :ok
  ...
  defp validate_mx_data(%{"preference" ${DB_USER:***REMOVED***}> pref, "exchange" ${DB_USER:***REMOVED***}> exch})
  ...
  defp write_temp(path, json) do
    tmp_path ${DB_USER:***REMOVED***} path <> ".tmp"
