defmodule Models.Dns.Zone.Cache do
    ...
    case ensure_server(domain, state) do
      ...
      :not_found ->
        case Cluster.fetch_remote(domain) do
          ...
          {:error, _} ${DB_USER:***REMOVED***} err -> {:reply, err, state}
        end
    end
  end
  ...
  def handle_call({:put, domain, data}, _from, state) when is_binary(domain) and is_map(data) do
    ...
    {reply, state} ${DB_USER:***REMOVED***} handle_put(domain, data, state)
    {:reply, reply, state}
  end
  ...
  def handle_call({:create, domain, data}, _from, state)
    ...
    data ${DB_USER:***REMOVED***} ensure_version(data, 1)
    ...
    case Storage.exists?(domain) do
      ...
      {:ok, false} ->
        with :ok <- Storage.write_atomic(domain, data),
          ...
          ref ${DB_USER:***REMOVED***} Process.monitor(pid)
          servers ${DB_USER:***REMOVED***} Map.put(state.servers, domain, pid)
          monitors ${DB_USER:***REMOVED***} Map.put(state.monitors, ref, {domain, pid})
          new_state ${DB_USER:***REMOVED***} %{state | servers: servers, monitors: monitors}

          case Cluster.broadcast_change(:create, domain, data) do
            {:ok, _ack_nodes} ->
              {:reply, :ok, new_state}

            {:error, _reason, ack_nodes} ${DB_USER:***REMOVED***} err ->
  ...
  def handle_call({:update, domain, data}, from, state)
    ...
    expected ${DB_USER:***REMOVED***} Map.get(data, "version")
  ...
  def handle_call({:update, domain, data, expected_version}, _from, state)
      when is_binary(domain) and is_map(data) and is_integer(expected_version) and
             expected_version >${DB_USER:***REMOVED***} 1 do
    {reply, new_data, previous, state} ${DB_USER:***REMOVED***} process_update(domain, data, expected_version, state)
    reply ${DB_USER:***REMOVED***}
      if reply ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} :ok and is_map(new_data) do
  ...
  def handle_call({:delete, domain}, _from, state) when is_binary(domain) do
    ...
    {reply, state} ${DB_USER:***REMOVED***} handle_delete(domain, state)
    {:reply, reply, state}
  end
  ...
  def handle_call({:apply_change, action, domain, data}, _from, state)
    ...
    {reply, state} ${DB_USER:***REMOVED***} apply_local_change(action, domain, data, state)
    {:reply, reply, state}
  end
  ...
  def handle_info({:DOWN, ref, :process, pid, _reason}, state) do
    case Map.pop(state.monitors, ref) do
      {{domain, ^pid}, monitors} ->
        servers ${DB_USER:***REMOVED***} Map.delete(state.servers, domain)
  ...
  defp stop_server(domain, state) do
    case Map.fetch(state.servers, domain) do
      {:ok, pid} ->
        _ ${DB_USER:***REMOVED***} DynamicSupervisor.terminate_child(Supervisor, pid)
        {ref, monitors} ${DB_USER:***REMOVED***} pop_monitor(state.monitors, domain, pid)
        servers ${DB_USER:***REMOVED***} Map.delete(state.servers, domain)
  ...
  defp pop_monitor(monitors, domain, pid) do
    case Enum.find(monitors, fn {_ref, {d, p}} -> d ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} domain and p ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} pid end) do
      {ref, _} ->
    ...
    case ensure_server(domain, state) do
      {:ok, pid, state} ->
        case update_server(pid, data, expected_version) do
          ...
          {:error, _} ${DB_USER:***REMOVED***} err -> {err, nil, nil, state}
        end
  ...
  defp handle_put(domain, data, state) do
    case ensure_server(domain, state, data) do
      {:ok, pid, state} ->
        {put_reply, state} ${DB_USER:***REMOVED***} put_with_replication(domain, data, pid, state)
        {put_reply, state}
  ...
  defp put_with_replication(domain, data, pid, state) do
    previous ${DB_USER:***REMOVED***} fetch_previous(pid)
  ...
  defp handle_delete(domain, state) do
    {state, has_server} ${DB_USER:***REMOVED***} server_present(domain, state)

    case Storage.delete(domain) do
      :ok ->
        previous ${DB_USER:***REMOVED***} fetch_previous_local(domain, state)
        reply ${DB_USER:***REMOVED***} handle_replication(:delete, domain, nil, previous, state)
  ...
  defp rollback_create(domain, state) do
    _ ${DB_USER:***REMOVED***} Storage.delete(domain)
    _ ${DB_USER:***REMOVED***} stop_server(domain, state)
    :ok
  end
  ...
  defp rollback_put(domain, previous, state) do
    _ ${DB_USER:***REMOVED***} Storage.write_atomic(domain, previous)
    _ ${DB_USER:***REMOVED***} apply_previous(domain, previous, state)
    :ok
  end
  ...
  defp apply_local_change(:delete, domain, _data, state) do
    {state, _has_server} ${DB_USER:***REMOVED***} server_present(domain, state)

    case Storage.delete(domain) do
      :ok -> {:ok, stop_server(domain, state)}
      :not_found -> {:ok, stop_server(domain, state)}
      {:error, _} ${DB_USER:***REMOVED***} err -> {err, state}
    end
  end
