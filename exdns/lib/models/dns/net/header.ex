defmodule Models.Dns.Net.Header do
  ...
  def deserialize(binary) when is_bitstring(binary) do
    Logger.debug("[Header] deserialize size${DB_USER:***REMOVED***}#{byte_size(binary)}")
    <<
      id::16,
      qr::1,
      opcode::4,
  ...
  def serialize(%Header{} ${DB_USER:***REMOVED***} header) do
    with {:ok, header} <- validate_header(header) do
      binary ${DB_USER:***REMOVED***}
        <<
          header.id::16,
          header.qr::1,
          header.opcode::4,
      ...
      Logger.debug("[Header] serialize id${DB_USER:***REMOVED***}#{header.id}")
      {:ok, binary}
    end
  end
  ...
  defp validate_header(%Header{} ${DB_USER:***REMOVED***} header) do
    {ok_fields, errors} ${DB_USER:***REMOVED***}
      header
      |> Map.from_struct()
