defmodule Models.Dns.Net.Question do
  ...
  def deserialize(message, offset \\ 0)
      when is_binary(message) and is_integer(offset) and offset >${DB_USER:***REMOVED***} 0 do
    Logger.debug("[Question] deserialize offset${DB_USER:***REMOVED***}#{offset}")
    with {:ok, qname, offset1} <- Name.decode(message, offset),
      ...
      Logger.debug("[Question] deserialize qname${DB_USER:***REMOVED***}#{qname} qtype${DB_USER:***REMOVED***}#{qtype} qclass${DB_USER:***REMOVED***}#{qclass}")
  ...
  def serialize(%Question{} ${DB_USER:***REMOVED***} question) do
    Logger.debug("[Question] serialize qname${DB_USER:***REMOVED***}#{question.qname} qtype${DB_USER:***REMOVED***}#{question.qtype}")
  ...
  defp parse_qtype_qclass(message, offset) do
    if byte_size(message) < offset + 4 do
      ...
      <<_::binary-size(offset), qtype::16, qclass::16, _::binary>> ${DB_USER:***REMOVED***} message
  ...
  defp validate_qname(qname) do
    cond do
      ...
      qname !${DB_USER:***REMOVED***} "." and Enum.any?(String.split(qname, ".", trim: true), &(byte_size(&1) > 63)) ->
        {:error, "each label in qname must be <${DB_USER:***REMOVED***} 63 bytes"}
  ...
  defp validate_qtype(qtype) when is_integer(qtype) and qtype >${DB_USER:***REMOVED***} 0 and qtype <${DB_USER:***REMOVED***} 0xFFFF, do: :ok
  defp validate_qtype(_), do: {:error, "qtype must be between 0 and 65535"}

  defp validate_qclass(qclass) when is_integer(qclass) and qclass >${DB_USER:***REMOVED***} 0 and qclass <${DB_USER:***REMOVED***} 0xFFFF,
    do: :ok
