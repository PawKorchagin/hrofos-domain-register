defmodule Models.Dns.Net.Request do
  ...
  def deserialize(message) when is_binary(message) do
    Logger.debug("[Request] deserialize size${DB_USER:***REMOVED***}#{byte_size(message)}")
    if byte_size(message) < 12 do
      {:error, "dns message truncated (missing header)"}
    else
      <<header_bin::binary-size(12), _rest::binary>> ${DB_USER:***REMOVED***} message
      ...
      with {:ok, header} <- Header.deserialize(header_bin),
        ...
        Logger.debug("[Request] deserialize ok q${DB_USER:***REMOVED***}#{length(questions)} a${DB_USER:***REMOVED***}#{length(answers)} ns${DB_USER:***REMOVED***}#{length(authorities)} ar${DB_USER:***REMOVED***}#{length(additional.records)}")
        result
      end
    end
  end
  ...
  def serialize(%__MODULE__{} ${DB_USER:***REMOVED***} request) do
    Logger.debug("[Request] serialize q${DB_USER:***REMOVED***}#{length(request.question || [])} a${DB_USER:***REMOVED***}#{length(request.answer || [])}")
    with {:ok, header} <- prepare_header(request),
      ...
      payload ${DB_USER:***REMOVED***} IO.iodata_to_binary([header_bin, question_bin, answer_bin, authority_bin, additional_bin])
      Logger.debug("[Request] serialize size${DB_USER:***REMOVED***}#{byte_size(payload)}")
      {:ok, payload}
    end
  end
  ...
  defp ensure_consumed(message, offset) do
    if offset ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} byte_size(message) do
      :ok
    else
  ...
  defp prepare_header(%__MODULE__{header: %Header{} ${DB_USER:***REMOVED***} header} ${DB_USER:***REMOVED***} request) do
  ...
  defp serialize_additional(%Additional{} ${DB_USER:***REMOVED***} add), do: Additional.serialize(add)
  defp serialize_additional(_), do: {:error, "additional section must be a %Additional{} struct"}

  defp additional_rr_count(%Additional{} ${DB_USER:***REMOVED***} additional) do
