defmodule Models.Dns.Rr.Common do
  ...
  defguardp is_octet(value) when is_integer(value) and value >${DB_USER:***REMOVED***} 0 and value <${DB_USER:***REMOVED***} 255
  defguardp is_u16(value) when is_integer(value) and value >${DB_USER:***REMOVED***} 0 and value <${DB_USER:***REMOVED***} 0xFFFF
  ...
  def encode_txt_strings(strings) when is_list(strings) do
    reducer ${DB_USER:***REMOVED***} fn string, {:ok, acc} ->
  ...
  defp do_decode_txt(<<len, rest::binary>>, acc) do
    if byte_size(rest) < len do
      ...
      <<chunk::binary-size(len), tail::binary>> ${DB_USER:***REMOVED***} rest
  ...
  defp read_fixed_header(message, offset) do
    ...
    if remaining < 10 do
      ...
      <<type::16, class::16, ttl::32, rdlength::16, _::binary>> ${DB_USER:***REMOVED***}
