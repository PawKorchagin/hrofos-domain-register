defmodule Models.Dns.Rr.A do
  @moduledoc false
  alias Models.Dns.Rr.Common

  @type t :: %__MODULE__{
          name: String.t(),
          class: non_neg_integer(),
          ttl: non_neg_integer(),
          address: :inet.ip4_address()
        }

  defstruct name: ".", class: 1, ttl: 0, address: {0, 0, 0, 0}

  @type_code 1

  @doc """
  Serialize an IPv4 A record to DNS wire format.
  """
  @spec serialize(t()) :: {:ok, binary()} | {:error, String.t()}
  def serialize(%__MODULE__{} ${DB_USER:***REMOVED***} rr) do
    with {:ok, rdata} <- Common.encode_ipv4(rr.address) do
      Common.serialize_rr(rr, @type_code, rdata)
    end
  end

  @doc """
  Deserialize an IPv4 A record starting at `offset` in the DNS `message`.
  """
  @spec deserialize(binary(), non_neg_integer()) ::
          {:ok, t(), non_neg_integer()} | {:error, String.t()}
  def deserialize(message, offset \\ 0) do
    with {:ok, header} <- Common.deserialize_rr(message, offset, @type_code),
         {:ok, address} <-
           message
           |> binary_part(header.rdata_offset, header.rdlength)
           |> Common.decode_ipv4() do
      rr ${DB_USER:***REMOVED***} %__MODULE__{
        name: header.name,
        class: header.class,
        ttl: header.ttl,
        address: address
      }

      {:ok, rr, header.next_offset}
    end
  end
end
