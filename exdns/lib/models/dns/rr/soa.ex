defmodule Models.Dns.Rr.Soa do
  ...
  def serialize(%__MODULE__{} ${DB_USER:***REMOVED***} rr) do
    with {:ok, mname_wire} <- Name.encode(rr.mname),
      ...
      rdata ${DB_USER:***REMOVED***} mname_wire <> rname_wire <> timers
  ...
  defp decode_rdata(message, header) do
    cursor ${DB_USER:***REMOVED***} header.rdata_offset
  ...
  defp decode_timers(message, cursor) do
    remaining ${DB_USER:***REMOVED***} byte_size(message) - cursor

    if remaining < 20 do
      {:error, "soa rdata truncated"}
    else
      <<serial::32, refresh::32, retry::32, expire::32, minimum::32>> ${DB_USER:***REMOVED***}
