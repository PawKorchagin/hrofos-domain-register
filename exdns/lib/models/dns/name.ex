defmodule Models.Dns.Name do
  ...
  def encode(name) when is_binary(name) do
    Logger.debug("[Name] encode name${DB_USER:***REMOVED***}#{name}")
    trimmed ${DB_USER:***REMOVED***} String.trim_trailing(name, ".")
    labels ${DB_USER:***REMOVED***} trimmed |> String.split(".", trim: true)

    cond do
      byte_size(trimmed) > 0 and labels ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} [] ->
  ...
  def decode(message, offset) when is_binary(message) and is_integer(offset) and offset >${DB_USER:***REMOVED***} 0 do
    Logger.debug("[Name] decode offset${DB_USER:***REMOVED***}#{offset}")
  ...
  defp parse_labels(message, offset, visited, jumps) do
    with :ok <- check_jumps(jumps),
      ...
      b ${DB_USER:***REMOVED***} :binary.at(message, offset)
  ...
  defp check_bounds(message, offset) do
    if offset >${DB_USER:***REMOVED***} byte_size(message) do
  ...
  defp parse_label_type(b, message, offset, visited, jumps)
       when band(b, 0b1100_0000) ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} 0b1100_0000 do
    parse_pointer(message, offset, visited, jumps, b)
  end

  defp parse_label_type(b, message, offset, visited, jumps) when b <${DB_USER:***REMOVED***} 63 do
  ...
  defp parse_pointer(message, offset, visited, jumps, b) do
    if offset + 1 >${DB_USER:***REMOVED***} byte_size(message) do
      {:error, "truncated compression pointer at offset #{offset}"}
    else
      b2 ${DB_USER:***REMOVED***} :binary.at(message, offset + 1)
      ptr ${DB_USER:***REMOVED***} bor(bsl(band(b, 0b0011_1111), 8), b2)

      cond do
        MapSet.member?(visited, ptr) ->
          {:error, "compression pointer loop detected"}

        ptr >${DB_USER:***REMOVED***} byte_size(message) ->
          {:error, "compression pointer #{ptr} outside message"}

        true ->
          new_visited ${DB_USER:***REMOVED***} MapSet.put(visited, ptr)
  ...
  defp parse_label(message, offset, visited, jumps, label_len) do
    label_start ${DB_USER:***REMOVED***} offset + 1
    label_end ${DB_USER:***REMOVED***} label_start + label_len

    if label_end > byte_size(message) do
      {:error, "truncated label at offset #{offset}"}
    else
      label ${DB_USER:***REMOVED***} binary_part(message, label_start, label_len)
