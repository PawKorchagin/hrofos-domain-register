defmodule Exdns.Http.Router do
  ...
  defp authorize(conn, _opts) do
    token ${DB_USER:***REMOVED***}
      :exdns
      |> Application.fetch_env!(:api_token)
      |> to_string()

    case Plug.Conn.get_req_header(conn, "authentication") do
      ["Bearer " <> provided] when provided ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} token ->
        conn
  ...
  defp cors(conn, _opts) do
    origin ${DB_USER:***REMOVED***} Plug.Conn.get_req_header(conn, "origin") |> List.first()
    allowed ${DB_USER:***REMOVED***} Application.get_env(:exdns, :cors_origin, "*")
    allow_origin ${DB_USER:***REMOVED***}
      cond do
        allowed ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} "*" -> "*"
        is_list(allowed) and origin in allowed -> origin
        is_binary(allowed) -> allowed
        true -> nil
      end

    conn ${DB_USER:***REMOVED***}
      if allow_origin do
        conn
    ...
    if conn.method ${DB_USER:***REMOVED***}${DB_USER:***REMOVED***} "OPTIONS" do
      conn
  ...
  defp json(conn, status, body) do
    payload ${DB_USER:***REMOVED***} Jason.encode!(body)
